...
export function usePlayerRankings() {
  ...
  /**
   * Calculate drill rankings for a specific player within their age group
   */
  const calculateDrillRankings = useMemo(() => {
      ...
      DRILLS.forEach(drill => {
        try {
          ...
          if (import.meta.env.DEV) {
            console.warn('[Rankings] Drill ranking calculation failed for', drill.key, ':', error);
          }
          rankings[drill.key] = null;
        }
      });
    ...
    return (player, allPlayers, currentWeights) => {
      ...
      return DRILLS.map(drill => {
        try {
          ...
          if (import.meta.env.DEV) {
            console.warn('[Rankings] Weighted breakdown calculation failed for', drill.key, ':', error);
          }
          return {
            ...drill,
            rawScore: null,
  ...
  const calculatePlayerRank = useCallback((player, allPlayers, currentWeights) => {
    ...
    try {
      ...
      if (ageGroupPlayers.length > 0) {
        ...
        const playersWithScores = ageGroupPlayers.map(p => {
          try {
            ...
            if (import.meta.env.DEV) {
              console.warn('[Rankings] Player score calculation failed:', error);
            }
      ...
      if (import.meta.env.DEV) {
        console.warn('[Rankings] Ranking calculation failed:', error);
      }
      currentRank = 1;
