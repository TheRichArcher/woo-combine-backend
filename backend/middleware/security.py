...
class SecurityHeadersMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        try:
            force_https_env = os.getenv("FORCE_HTTPS", "true").lower() in (
                "1",
                "true",
                "yes",
            )
        ...
        is_auth_endpoint = request.url.path in [
            ...
            "/api/warmup",
            "/api/health",
        ] or request.url.path.startswith("/api/leagues/me")
    ...
    def add_security_headers(self, response: Response, request: Request):
        ...
        report_only = (
            os.getenv("CSP_REPORT_ONLY", "false").lower() in ("1", "true", "yes")
            or env == "staging"
        )
        ...
        allow_unsafe_inline_scripts = os.getenv(
            "CSP_ALLOW_UNSAFE_INLINE_SCRIPTS", "false"
        ).lower() in ("1", "true", "yes")

        script_src_values = ["'self'"] + (
            ["'unsafe-inline'"] if allow_unsafe_inline_scripts else []
        )
        ...
        extra_connect = [
            v.strip() for v in os.getenv("CSP_CONNECT_SRC", "").split(",") if v.strip()
        ]
        ...
        if not is_auth_endpoint:
            ...
            if hasattr(request, "headers"):
                ...
                if content_length and int(content_length) > self.max_request_size:
                    logging.warning(
                        f"Request too large: {content_length} bytes from {request.client}"
                    )
                    return Response(
            ...
            if self.is_suspicious_path(request.url.path):
                logging.warning(
                    f"Suspicious request path: {request.url.path} from {request.client}"
                )
                return Response(
            ...
            if self.is_suspicious_user_agent(user_agent):
                logging.warning(
                    f"Suspicious user agent: {user_agent} from {request.client}"
                )
                return Response(
                    content="Invalid request",
                    status_code=400,
...
def add_security_middleware(app, config=None):
    ...
    logging.warning(
        "add_security_middleware is deprecated. Use add_security_headers_middleware and add_request_validation_middleware to control ordering."
    )
