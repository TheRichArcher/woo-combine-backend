...
RATE_LIMITS = {
    ...
    "auth": _normalize_rate_string(
        os.getenv("RATE_LIMITS_AUTH", ""), _DEFAULTS["auth"]
    ),
    # User management (optional override; fallback to default if not set)
    "users": _normalize_rate_string(
        os.getenv("RATE_LIMITS_USERS", ""), _DEFAULTS["users"]
    ),
    # Data retrieval
    "read": _normalize_rate_string(
        os.getenv("RATE_LIMITS_READ", ""), _DEFAULTS["read"]
    ),
    # Data creation/updates
    "write": _normalize_rate_string(
        os.getenv("RATE_LIMITS_WRITE", ""), _DEFAULTS["write"]
    ),
    # Bulk operations
    "bulk": _normalize_rate_string(
        os.getenv("RATE_LIMITS_BULK", ""), _DEFAULTS["bulk"]
    ),
    # Health checks
    "health": _normalize_rate_string(
        os.getenv("RATE_LIMITS_HEALTH", ""), _DEFAULTS["health"]
    ),
}
...
def create_rate_limit_handler():
    ...
    async def rate_limit_handler(request: Request, exc: RateLimitExceeded):
        ...
        if (
            hasattr(request.app.state, "limiter")
            and hasattr(request, "state")
            and hasattr(request.state, "view_rate_limit")
        ):
