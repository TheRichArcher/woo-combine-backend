...
try:
    if not firebase_admin._apps:
        ...
        logging.info(
            f"[AUTH] GOOGLE_APPLICATION_CREDENTIALS_JSON exists: {bool(creds_json)}"
        )
        logging.info(
            f"[AUTH] GOOGLE_APPLICATION_CREDENTIALS exists: {bool(creds_path)}"
        )
        if creds_path:
            logging.info(
                f"[AUTH] GOOGLE_APPLICATION_CREDENTIALS value starts with: {creds_path[:50]}..."
            )
        ...
        try:
            ...
            logging.warning(
                f"[AUTH] Firebase initialize_app failed, continuing in degraded mode: {e}"
            )
except Exception as e:
...
AUTH_CHECK_REVOKED = os.getenv("AUTH_CHECK_REVOKED", "true").lower() in (
    "1",
    "true",
    "yes",
    "on",
...
_auth_failure_tracker: defaultdict[str, Dict[str, float]] = defaultdict(
...
def _track_auth_failure(reason: str) -> None:
    ...
    if info["count"] in (5, 10, 20):
        logging.warning(
            "[AUTH] Repeated auth failures for %s (%s in last 5m)",
            reason,
            info["count"],
        )
...
def _enforce_session_max_age(decoded_token: dict):
    ...
    try:
        ...
        if auth_time and now - auth_time > max_age_secs:
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED, detail="Session too old"
            )
...
REQUIRE_VERIFIED_LOGIN = os.getenv("REQUIRE_VERIFIED_LOGIN", "false").lower() in (
    "1",
    "true",
    "yes",
    "on",
...
def get_current_user(
    ...
    try:
        ...
        try:
            ...
            logging.info(
                f"[AUTH] Token verification successful for uid: {decoded_token.get('uid')}"
            )
        except getattr(auth, "RevokedIdTokenError", Exception) as revoked_err:
            logging.warning(
                f"[AUTH] Token has been revoked: {type(revoked_err).__name__}"
            )
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
            ...
            logging.error(
                f"[AUTH] Firebase token verification failed ({err_type}): {e}"
            )
            _track_auth_failure("token_verification")
            if AUTH_CHECK_REVOKED and AUTH_FALLBACK_NO_REVOKE:
                try:
                    logging.warning(
                        "[AUTH] Retrying token verification without revocation check due to error above"
                    )
                    decoded_token = _verify_id_token(token, check_revoked=False)
                    logging.info(
                        "[AUTH] Token verification succeeded without revocation check"
                    )
                except Exception as e2:
                    err_type2 = type(e2).__name__
                    logging.error(
                        f"[AUTH] Fallback token verification failed ({err_type2}): {e2}"
                    )
                    raise HTTPException(
                        status_code=status.HTTP_401_UNAUTHORIZED,
        ...
        logging.info(
            f"[AUTH] Decoded Firebase token for UID: {decoded_token.get('uid')}, email: {decoded_token.get('email')}"
        )
        ...
        try:
            ...
            logging.info(
                f"[AUTH] Firestore lookup completed successfully. User exists: {user_doc.exists}"
            )
        ...
        if not user_doc.exists:
            ...
            logging.info(
                f"[AUTH] Creating new user document for UID {uid} to enable guided setup"
            )
            try:
        ...
        if not role:
            logging.info(
                f"[AUTH] User with UID {uid} found but no role set - allowing SelectRole access"
            )
            return {
                "uid": uid,
                "email": stored_email,
    ...
    try:
        ...
        if not email_verified:
            ...
            if current_time - token_issued_at < 300:  # 5 minutes
                logging.info(
                    "[AUTH] Allowing role setting for recent token (might be verification delay)"
                )
            else:
                logging.warning(
                    f"[AUTH] User {uid} has not verified their email (for role setting)"
                )
                raise HTTPException(
        ...
        try:
            ...
            user_doc = db.collection("users").document(uid).get()
            logging.info(
                f"[AUTH] Firestore lookup completed. User exists: {user_doc.exists}"
            )
        ...
        if not user_doc.exists:
            ...
            try:
                ...
                db.collection("users").document(uid).set(user_data)
                logging.info(
                    f"[AUTH] Successfully created user document for role setting: {uid}"
                )
                return {"uid": uid, "email": email, "role": None}

            except Exception as create_error:
                logging.error(
                    f"[AUTH] Failed to create user document for role setting: {create_error}"
                )
                logging.error(
                    f"[AUTH] Create error type: {type(create_error).__name__}"
                )
