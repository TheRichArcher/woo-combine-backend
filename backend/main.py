...
_ENABLE_DEBUG_ENDPOINTS = os.getenv("ENABLE_DEBUG_ENDPOINTS", "false").lower() in (
    "1",
    "true",
    "yes",
)
    ...
    if _firestore_client is None:
        try:
            ...
            if "DefaultCredentialsError" in str(e) or "credentials" in str(e).lower():
                logging.error(
                    "[STARTUP] ❗ Missing Firebase credentials! Set GOOGLE_APPLICATION_CREDENTIALS_JSON in environment"
                )
            _firestore_client = None
...
def meta():
    return {
        ...
        "role_simple_enabled": os.getenv("ENABLE_ROLE_SIMPLE", "false").lower()
        in ("1", "true", "yes"),
    }
...
def security_txt():
    ...
    policy_url = os.getenv(
        "SECURITY_POLICY_URL", "https://www.woo-combine.com/security"
    )
    acknowledgments = os.getenv(
        "SECURITY_ACK_URL", "https://www.woo-combine.com/hall-of-fame"
    )
        ...
        try:
            ...
            def fetch_one(col_name: str):
                try:
                    ...
                    logging.warning(
                        f"[WARMUP] Warmup fetch failed for {col_name}: {exc}"
                    )
                    return None
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
                ...
                for f in futures:
                    try:
                        ...
                        logging.warning(
                            f"[WARMUP] Subtask timed out or failed: {sub_exc}"
                        )
...
if _ENABLE_DEBUG_ENDPOINTS:
    ...
    def test_auth_debug(current_user=Depends(get_current_user)):
        ...
        try:
            logging.info(
                f"[TEST-AUTH] Auth test called by user: {current_user.get('uid', 'unknown')}"
            )
            return {
                "status": "success",
                "user": current_user.get("uid", "unknown"),
...
if DIST_DIR.exists():
    logging.info(
        f"[STARTUP] Frontend directory found but NOT serving (API-only mode): {DIST_DIR}"
    )
else:
