...
async def get_draft_pricing(draft_id: str, user: dict = Depends(get_current_user)) -> PricingResponse:
...
async def create_checkout_session(
    ...
    if not PAYMENTS_ENABLED:
        raise HTTPException(status_code=400, detail="Payments not enabled - drafts are free")
...
async def bypass_payment(draft_id: str, user: dict = Depends(get_current_user)):
    ...
    """
    if PAYMENTS_ENABLED:
        raise HTTPException(status_code=403, detail="Cannot bypass when payments enabled")
