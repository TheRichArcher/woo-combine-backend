...
def get_batch_players(
    ...
    dry_run: bool = Query(
        False, description="Preview payload without fetching players"
    ),
):
    """
    ...
    try:
        ...
        batch_results = {}
        summary = {
            "requested": payload.event_ids[:MAX_ITEMS_PER_BATCH],
            "validated": [],
            "missing": [],
        }
...
def get_batch_events(
    ...
    try:
        ...
        batch_results = {}
        summary = {
            "requested": payload.league_ids[:MAX_ITEMS_PER_BATCH],
            "validated": [],
            "missing": [],
        }
...
def get_dashboard_data(
    request: Request, league_id: str, current_user=Depends(get_current_user)
):
    """
    ...
    try:
        ...
        if league_doc.exists:
            dashboard_data["league"] = {**league_doc.to_dict(), "id": league_doc.id}
        else:
            raise HTTPException(status_code=404, detail="League not found")

        # Get events for this league
        events_ref = (
            db.collection("leagues").document(str(league_id)).collection("events")
        )
        events_stream = execute_with_timeout(
            lambda: list(events_ref.stream()),
            timeout=8,
        ...
        dashboard_data["player_stats"] = {
            ...
            "avg_players_per_event": (
                round(total_players / len(events), 1) if events else 0
            ),
        }
