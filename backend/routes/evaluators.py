...
def get_event_evaluators(
    request: Request, event_id: str, current_user=Depends(get_current_user)
):
    """Get all evaluators for an event"""
    try:
        enforce_event_league_relationship(event_id=event_id)
        evaluators_ref = (
            db.collection("events").document(event_id).collection("evaluators")
        )
...
@write_rate_limit()
@require_permission(
    "evaluators", "submit_evaluation", target="event", target_param="event_id"
)
def submit_drill_evaluation(
    ...
    try:
        ...
        validated_value = validate_drill_score(
            float(evaluation.value), evaluation.drill_type
        )
        ...
        evaluations_ref = (
            db.collection("events").document(event_id).collection("drill_evaluations")
        )
        ...
        _update_aggregated_drill_results(
            event_id, evaluation.player_id, evaluation.drill_type
        )

        logging.info(
            f"Submitted evaluation for player {evaluation.player_id}, drill {evaluation.drill_type}"
        )
...
@read_rate_limit()
@require_permission(
    "evaluators", "player_evaluations", target="event", target_param="event_id"
)
...
@read_rate_limit()
@require_permission(
    "evaluators", "aggregated_results", target="event", target_param="event_id"
)
...
def _update_aggregated_drill_results(event_id: str, player_id: str, drill_type: str):
    ...
    try:
        ...
        evaluations = execute_with_timeout(
            lambda: list(
                evaluations_ref.where("player_id", "==", player_id)
        ...
        logging.info(
            f"Updated aggregated result for player {player_id}, drill {drill_type}"
        )
