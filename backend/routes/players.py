...
def calculate_composite_score(
    ...
    schema: Optional[SportSchema] = None,
) -> float:
    """
    ...
    use_weights = (
        ...
        else {d.key: d.default_weight for d in schema.drills}
    )
...
def get_players(
    ...
    try:
        if "user_id" in request.query_params:
            raise HTTPException(
                status_code=400,
                detail="Do not include user_id in query params. Use Authorization header.",
            )
        ...
        def get_players_query():
            query = (
                db.collection("events").document(str(event_id)).collection("players")
            )
        ...
        for player in players_stream:
            ...
            player_dict["composite_score"] = calculate_composite_score(
                player_dict, schema=schema
            )
...
def create_player(
    ...
    try:
        logging.info(
            f"[CREATE_PLAYER] Starting player creation for event_id: {event_id}"
        )

        # Validate player name is not empty
        if not player.name or not player.name.strip():
            raise HTTPException(
                status_code=422, detail="Player name is required and cannot be empty"
            )
        ...
        existing_player = execute_with_timeout(lambda: player_doc_ref.get(), timeout=3)
        ...
        logging.info(
            f"[CREATE_PLAYER] Creating player document with deterministic ID: {player_id}"
        )
        ...
        execute_with_timeout(lambda: player_doc_ref.set(player_data), timeout=5)
...
def update_player(
    ...
    try:
        ...
        if (
            not player_id
            or len(player_id) > 1500
            or any(c in player_id for c in ["/", "\x00"])
        ):
            raise HTTPException(
                status_code=422,
        ...
        player_doc = execute_with_timeout(player_ref.get, timeout=3)
        ...
        execute_with_timeout(lambda: player_ref.update(update_data), timeout=5)
...
class UploadRequest(BaseModel):
    ...
    mode: Optional[str] = (
        "create_or_update"  # Options: "create_or_update", "scores_only"
    )
...
def upload_players(
    ...
    try:
        ...
        if req.mode == "scores_only" and len(players) > 0:
            ...
            if (
                p0.get("scores")
                and isinstance(p0.get("scores"), dict)
                and len(p0.get("scores")) > 0
            ):
            ...
            if not has_potential_scores:
                logging.warning(
                    f"[IMPORT_GUARD] Blocked import. Mode={req.mode}, Payload Keys={list(p0.keys())}, Schema Keys={drill_fields}"
                )
                raise HTTPException(
                    status_code=400,
        ...
        if (
            ...
            logging.info(
                f"[IMPORT_START] Event={event_id}, Mode={req.mode}, Rows={len(players)}, Schema={drill_fields}, Row1_Keys={list(p0.keys())}"
            )
        added = 0
        created_players = 0
        updated_players = 0
        ...
        if len(players) > MAX_ROWS:
            raise HTTPException(
                status_code=400, detail=f"Too many rows: max {MAX_ROWS}"
            )
            ...
            if p.get("first_name") and p.get("last_name"):
                try:
                    ...
                    pid = generate_player_id(
                        event_id, p.get("first_name"), p.get("last_name"), num
                    )
            ...
            for i in range(0, len(unique_ids), 100):
                ...
                for doc in docs:
                    if doc.exists:
                        ...
                        existing_docs_map[doc.id] = (
                            None  # Explicitly mark as not existing
                        )
        ...
        if players:
            ...
            logging.info(
                f"[UPLOAD_RECEIPT] Received {len(players)} players for event {event_id}"
            )
            logging.info(
                f"[UPLOAD_RECEIPT] First player raw keys: {list(first_player.keys())}"
            )
            logging.info(
                f"[UPLOAD_RECEIPT] First player identity fields: first_name={first_player.get('first_name')}, last_name={first_player.get('last_name')}, number={first_player.get('number')}"
            )
        ...
        for idx, player in enumerate(players):
            ...
            try:
                ...
                if idx == 0:
                    logging.info(
                        f"[NUMBER_EXTRACT] Row 1: Extracted {num} from field '{num_source}' (raw: {raw_num})"
                    )

                # DEBUG: Log number extraction details
                if num is None:
                    logging.warning(
                        f"[NUMBER_EXTRACT] Row {idx + 1}: No number found! Checked: number, player_number, jersey, jersey_number, etc. Player data keys: {list(player.keys())}"
                    )
                else:
                    logging.info(
                        f"[NUMBER_EXTRACT] Row {idx + 1}: Extracted {num} from field '{num_source}' (raw_value='{raw_num}')"
                    )
                ...
                logging.error(
                    f"[NUMBER_EXTRACT] Row {idx + 1}: Exception during number extraction: {e}, player keys: {list(player.keys())}"
                )
            ...
            logging.info(
                f"[DEDUPE] Row {idx + 1}: Identity key = {key} (first={first_name}, last={last_name}, number={num})"
            )
            ...
            if key in seen_keys:
                ...
                if num is None:
                    ...
                    age_group
                    and first_player.get("age_group")
                    and age_group != first_player.get("age_group")
                ):
            ...
            if req.mode == "scores_only" and not previous_state:
                ...
                errors.append(
                    {
                        "row": idx + 1,
                        "message": f"Player match not found for {first_name} {last_name} (#{num}). strictly requiring existing player.",
                    }
                )
                continue
            ...
            player_data = {
                ...
                "number": num,
                "age_group": (
                    str(player.get("age_group")).strip()
                    if str(player.get("age_group") or "").strip() != ""
                    else None
                ),
            ...
            if idx == 0:
                ...
                logging.info(
                    f"[STORAGE]   - scores keys: {list(player_data.get('scores', {}).keys())}"
                )
                logging.info(
                    f"[STORAGE]   - operation: {'UPDATE (merge with existing)' if previous_state else 'CREATE (new player)'}"
                )
        ...
        logging.info(
            f"[IMPORT_SUMMARY] Event {event_id}: {scores_written_total} total scores written across {len(scores_written_by_drill)} drills"
        )
        ...
        if missing_drills and scores_written_total == 0:
            logging.warning(
                f"[IMPORT_WARNING] Expected drill keys not received in any player data: {missing_drills}"
            )
            logging.warning(
                "[IMPORT_WARNING] This could mean: 1) No players had scores for these drills, 2) CSV columns weren't mapped correctly, or 3) Column names didn't match drill keys/labels"
            )
            # Add detail about first row keys
            if len(players) > 0:
                logging.warning(
                    f"[IMPORT_WARNING] First row keys for debugging: {list(players[0].keys())}"
                )
        elif missing_drills:
            # Just info log if we have some data but not all drills (common partial import case)
            logging.info(
                f"[IMPORT_INFO] Some drills were not present in this import: {missing_drills}"
            )
        ...
        logging.error(
            f"[UPLOAD_ERROR] Event ID: {req.event_id if hasattr(req, 'event_id') else 'unknown'}"
        )
        logging.error(
            f"[UPLOAD_ERROR] Number of players in request: {len(req.players) if hasattr(req, 'players') and req.players else 0}"
        )
        # Return more detailed error to help with debugging
        raise HTTPException(
            status_code=500, detail=f"Failed to upload players: {str(e)}"
        )
...
def reset_players(
    event_id: str = Query(...), current_user=Depends(require_role("organizer"))
):
    """
    ...
    try:
        ...
        players_stream = execute_with_timeout(
            lambda: list(players_ref.stream()), timeout=15
        )
        ...
        for player in players_stream:
            ...
            drill_results_stream = execute_with_timeout(
                lambda: list(drill_results_ref.stream()), timeout=10
            )
        ...
        execute_with_timeout(
            lambda: event_ref.update({"live_entry_active": False}), timeout=5
        )
        ...
        agg_ref = (
            db.collection("events")
            .document(str(event_id))
            .collection("aggregated_drill_results")
        )
...
def get_rankings(
    ...
    try:
        ...
        for player in players_stream:
            ...
            if (
                age_group
                and age_group.upper() != "ALL"
                and player_data.get("age_group") != age_group
            ):
                continue
            ...
            composite_score = calculate_composite_score(
                player_data, weights=use_weights, schema=schema
            )
            ...
            response_obj = {
                ...
                "age_group": player_data.get(
                    "age_group"
                ),  # Added for UI consistency with Live Standings
                "external_id": player_data.get(
                    "external_id"
                ),  # Added for combine analytics
...
@write_rate_limit()
@require_permission(
    "league_players", "create", target="league", target_param="league_id"
)
def add_player(
    request: Request,
    league_id: str,
...
@require_permission(
    "league_players", "drill_results", target="league", target_param="league_id"
)
