...
def get_my_leagues(request: Request, current_user=Depends(get_current_user)):
    ...
    try:
        ...
        user_doc = execute_with_timeout(
            ...
            timeout=5,
            operation_name="user_memberships lookup",
        )
        ...
        if user_doc.exists and user_doc.to_dict().get("leagues"):
            ...
            if league_memberships:
                ...
                logging.info(
                    f"📊 Batch fetching {len(league_ids)} leagues for user {user_id}"
                )
                ...
                league_docs = execute_with_timeout(
                    ...
                    timeout=8,
                    operation_name="batch league get",
                )
                ...
                for league_doc in league_docs:
                    if league_doc.exists:
                        ...
                        logging.info(
                            f"  ✅ League {league_doc.id} ({league_data.get('name', 'Unknown')}) with role: {role}"
                        )
                ...
                if user_leagues and len(user_leagues) > 0:
                    ...
                    logging.info(
                        f"🚀 Fast path: returned {len(user_leagues)} leagues for user {user_id}"
                    )
                    return {"leagues": user_leagues}

        # LEGACY PATH: Only if new system has no data - OPTIMIZED for extreme cold starts
        logging.info(
            f"🔄 No user_memberships found, checking legacy system for user {user_id}"
        )
        ...
        for league_doc in all_leagues:
            ...
            try:
                ...
                member_doc = execute_with_timeout(
                    member_ref.get, timeout=5, operation_name="member lookup"
                )
                ...
                if member_doc.exists:
                    ...
                    logging.info(
                        f"  🔄 Found legacy membership: {league_id} ({league_data.get('name', 'Unknown')}) with role: {role}"
                    )

            except Exception as e:
                logging.warning(
                    f"Error checking legacy membership in league {league_id}: {str(e)}"
                )
                continue
        ...
        if not user_leagues or len(user_leagues) == 0:
            ...
            logging.info(
                f"No leagues found for user {user_id} - returning empty array (new user)"
            )
            return {"leagues": []}
        ...
        if migration_data:
            try:
                ...
                execute_with_timeout(
                    ...
                    timeout=6,
                    operation_name="user_memberships migrate",
                )
                logging.info(
                    f"✅ Migrated {len(migration_data)} leagues to new system for user {user_id}"
                )
            except Exception as e:
        ...
        logging.info(
            f"🎉 Legacy path: returned {len(user_leagues)} leagues for user {user_id}"
        )
        return {"leagues": user_leagues}
        ...
        if "timeout" in str(e).lower():
            raise HTTPException(
                status_code=504,
                detail="Database operation timed out during server startup. Please try again.",
            )
        else:
            raise HTTPException(
                status_code=500,
                detail="Failed to retrieve leagues due to server initialization",
            )
...
@write_rate_limit()
def create_league(
    request: Request, req: dict, current_user=Depends(require_role("organizer"))
):
    """
    Create a new league.
    ...
    try:
        ...
        logging.info(
            f"🎉 League created with id {league_ref.id} by user {user_id} using batch operation"
        )
...
def join_league(
    ...
    """
    logging.info(
        f"[POST] /leagues/join/{code} called by user: {current_user} with req: {req}"
    )
    ...
    if requested_role == "organizer":
        logging.warning(
            f"Security Alert: User {user_id} attempted to join as organizer via code. Blocked."
        )
        raise HTTPException(
            status_code=403,
            detail="Cannot join as organizer via public code. Please contact a league admin.",
        )
    ...
    try:
        ...
        if not league_doc.exists:
            logging.warning(
                f"League document does not exist for ID: {code}. Attempting event fallback."
            )
            ...
            logging.info(
                f"Treated join code {code} as event ID. Resolved league {fallback_league_id}"
            )
            ...
            if not league_doc.exists:
                logging.error(
                    f"Event {code} referenced missing league {resolved_league_id}"
                )
                raise HTTPException(
                    status_code=404, detail="League not found for this event code"
                )
        ...
        batch.commit()
        logging.info(
            f"[BATCH] Successfully committed membership writes for user {user_id}"
        )

        logging.info(
            f"User {user_id} joined league {resolved_league_id} as {role} using batch operation"
        )
        response_payload = {
            "joined": True,
            "league_id": resolved_league_id,
...
def list_teams(
    ...
    try:
        ...
        teams_stream = execute_with_timeout(
            lambda: list(teams_ref.stream()), timeout=8, operation_name="teams stream"
        )
...
def list_invitations(
    ...
    try:
        invitations_ref = (
            db.collection("leagues").document(league_id).collection("invitations")
        )
...
def get_league_member(
    ...
    try:
        ...
        try:
            # Fast path
            current_memberships_ref = db.collection("user_memberships").document(
                current_user_uid
            )
            ...
            logging.error(
                f"[SECURITY] Failed to determine current user's league role: {role_check_err}"
            )
        ...
        if not membership:
            ...
            raise HTTPException(
                status_code=404, detail="Member not found in this league"
            )
        ...
        logging.error(
            f"Error retrieving league member {member_id} in league {league_id}: {str(e)}"
        )
...
@write_rate_limit()
@require_permission(
    "league_members", "update", target="league", target_param="league_id"
)
def update_member_status(
    ...
    try:
        ...
        if disabled is None:
            raise HTTPException(
                status_code=400, detail="Missing 'disabled' field in body"
            )

        logging.info(
            f"[PATCH] Updating member {member_id} status in league {league_id} to disabled={disabled}"
        )
        ...
        if disabled:
            ...
            if member_doc.exists and member_doc.to_dict().get("role") == "organizer":
                ...
                orgs_ref = (
                    db.collection("leagues").document(league_id).collection("members")
                )
                ...
                if is_target_active and active_count <= 1:
                    raise HTTPException(
                        status_code=400,
                        detail="Cannot disable the last active organizer. Promote another member first.",
                    )
        ...
        if user_doc.exists:
            batch.update(
                user_membership_ref, {f"leagues.{league_id}.disabled": disabled}
            )
        else:
            logging.warning(
                f"User membership doc not found for {member_id}, skipping fast path update"
            )
...
def update_member_write_permission(
    ...
    try:
        ...
        if can_write is None:
            raise HTTPException(
                status_code=400, detail="Missing 'canWrite' field in body"
            )
        ...
        membership_update = {f"leagues.{league_id}.canWrite": can_write}
