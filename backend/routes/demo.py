...
def _require_demo_enabled_and_token(request: Request):
    ...
    provided = request.headers.get("X-DEMO-SEED-TOKEN") or request.headers.get(
        "x-demo-seed-token"
    )
    if not expected:
        raise HTTPException(
            status_code=500,
            detail="Demo seeding not configured (missing DEMO_SEED_TOKEN)",
        )
    if provided != expected:
        raise HTTPException(
            status_code=403, detail="Invalid or missing demo seed token"
        )
...
def seed_demo(request: Request):
    ...
    try:
        ...
        existing = list(
            leagues_ref.where("name", "==", "Demo League").limit(1).stream()
        )
        if existing:
        ...
        for spec in event_specs:
            ...
            # dedupe by name
            existing_evt = list(
                events_ref.where("name", "==", spec["name"]).limit(1).stream()
            )
            if existing_evt:
            ...
            execute_with_timeout(
                lambda: top_level_event_ref.set(event_data), timeout=10
            )
            event_ids.append(evt_id)
        ...
        all_players_by_event: Dict[str, List[Dict[str, Any]]] = {
        ...
        for idx, eid in enumerate(event_ids):
            ...
            count_for_event = (
                per_event
                if idx < len(event_ids) - 1
                else total_players - per_event * (len(event_ids) - 1)
            )
            for _ in range(count_for_event):
                ...
                jersey_counter += 1
                execute_with_timeout(
                    lambda d=player_data: player_doc.set(d), timeout=10
                )
        ...
        for eid in event_ids:
            evaluators_ref = (
                db.collection("events").document(eid).collection("evaluators")
            )
        ...
        for eid in event_ids:
            evaluations_ref = (
                db.collection("events").document(eid).collection("drill_evaluations")
            )
            aggregated_ref = (
            ...
            for p in players:
                ...
                for drill in _drill_types():
                    ...
                    for _ in range(attempts):
                        ...
                        eval_data = {
                            ...
                            "unit": (
                                ...
                                if drill == "40m_dash"
                                    ...
                                    "inches" if drill == "vertical_jump" else "points"
                                )
                            ),
                        ...
                        execute_with_timeout(
                            lambda d=eval_data: eval_doc.set(d), timeout=10
                        )
                    ...
                    execute_with_timeout(
                        lambda d=agg_payload: aggregated_ref.document(agg_doc_id).set(
                            d
                        ),
                        timeout=10,
                    )
                ...
                execute_with_timeout(
                    lambda u=snapshot_updates: player_ref.update(u), timeout=10
                )
