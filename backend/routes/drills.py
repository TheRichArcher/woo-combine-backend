...
def delete_drill_result(
    ...
    try:
        ...
        previous_results = execute_with_timeout(
            lambda: list(results_query.stream()), timeout=5
        )
        ...
        if previous_results:
            ...
            logging.info(
                f"Reverted {drill_type} for player {player_id} to {previous_value}"
            )
        else:
            ...
            logging.info(
                f"Removed {drill_type} for player {player_id} (no previous scores)"
            )
