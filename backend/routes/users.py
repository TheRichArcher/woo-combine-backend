...
async def set_user_role(
    ...
    try:
        ...
        if current_role == "organizer" and role != "organizer":
            logging.warning(
                f"Organizer {uid} attempted self-demotion to {role}. Blocked."
            )
            raise HTTPException(
                status_code=400,
        ...
        try:
            ...
            try:
                ...
                _ = (
                    db.collection("_test").document("connectivity").get()
                )  # Test connectivity
                ...
                raise HTTPException(
                    status_code=500, detail="Database connectivity issue"
                )
            ...
            try:
                ...
                logging.info(
                    f"Successfully retrieved user document. Exists: {user_doc.exists}"
                )
            ...
            try:
                if user_doc.exists:
                    ...
                    logging.info(
                        f"✅ Created new user document for {uid} with role: {role}"
                    )
                ...
                raise HTTPException(
                    status_code=500, detail="Failed to save role to database"
                )
...
async def debug_set_user_role(
    ...
    try:
        ...
        if os.getenv("ENABLE_DEBUG_ENDPOINTS", "false").lower() not in (
            "1",
            "true",
            "yes",
        ):
...
@auth_rate_limit()
async def set_user_role_simple(role_data: SetRoleRequest, request: Request):
    """
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Database operation failed: {str(db_error)}"
            )
