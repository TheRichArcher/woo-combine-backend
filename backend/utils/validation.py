...
def canonicalize_age_group(value: str) -> str:
    ...
    raise ValidationError(
        "Invalid age group. Allowed values: " + ", ".join(ALLOWED_AGE_GROUPS)
    )
...
def validate_player_name(name: str) -> str:
    ...
    if not PATTERNS["player_name"].match(name):
        raise ValidationError(
            "Player name contains invalid characters. Only letters, spaces, hyphens, apostrophes, and periods are allowed"
        )
...
def validate_league_name(name: str) -> str:
    ...
    if not PATTERNS["league_name"].match(name):
        raise ValidationError(
            "League name contains invalid characters. Only letters, numbers, spaces, hyphens, apostrophes, periods, and parentheses are allowed"
        )
...
def validate_user_role(role: str) -> str:
    ...
    if role not in VALID_ROLES:
        raise ValidationError(
            f"Invalid user role. Must be one of: {', '.join(VALID_ROLES)}"
        )
...
class Validator:
    ...
    def drill_score(value: float, drill_type: str) -> float:
        ...
        Validator.number_range(
            value, f"{drill_type} score", ranges["min"], ranges["max"]
        )
...
def validate_player_data(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    validated["name"] = Validator.pattern_match(
        validated["name"], "name", "player_name"
    )

    # Optional fields
    if "number" in data and data["number"] is not None:
        validated["number"] = Validator.number_range(
            int(data["number"]), "number", 1, 9999
        )
    ...
    for drill_type in DRILL_SCORE_RANGES.keys():
        if drill_type in data and data[drill_type] is not None:
            validated[drill_type] = Validator.drill_score(
                float(data[drill_type]), drill_type
            )
...
def validate_league_data(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    validated["name"] = Validator.required(data.get("name"), "name")
    validated["name"] = Validator.pattern_match(
        validated["name"], "name", "league_name"
    )
...
def validate_event_data(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "location" in data and data["location"]:
        validated["location"] = Validator.string_length(
            data["location"], "location", 0, 200
        )
