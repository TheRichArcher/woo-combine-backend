...
def generate_delete_intent_token(
    user_id: str, league_id: str, target_event_id: str
) -> str:
    """
    ...
    if not SECRET_KEY:
        raise RuntimeError(
            "DELETE_TOKEN_SECRET_KEY not configured. Cannot generate tokens."
        )
    ...
    logging.info(
        f"[DELETE_TOKEN] Generated one-time token (jti: {jti}) for user {user_id}, event {target_event_id}, expires in {TOKEN_EXPIRY_MINUTES}min"
    )
...
def validate_delete_intent_token(
    ...
    if not SECRET_KEY:
        raise RuntimeError(
            "DELETE_TOKEN_SECRET_KEY not configured. Cannot validate tokens."
        )
    ...
    try:
        ...
        if not jti:
            ...
            raise ValueError(
                "Token missing jti claim (not a valid delete intent token)"
            )

        # Check if jti exists in usage store
        if jti not in _token_usage_store:
            logging.error(
                f"[DELETE_TOKEN] Unknown jti: {jti} - Token may be forged or expired cleanup occurred"
            )
        ...
        if token_record["used_at"] is not None:
            logging.error(
                f"[DELETE_TOKEN] REPLAY ATTACK DETECTED - jti: {jti} already used at {token_record['used_at']}"
            )
            raise ValueError(
                f"Token already used at {token_record['used_at'].isoformat()}. Replay attacks are blocked."
            )
        ...
        if payload.get("user_id") != expected_user_id:
            logging.error(
                f"[DELETE_TOKEN] User ID mismatch - Token: {payload.get('user_id')}, Expected: {expected_user_id}"
            )
        ...
        if payload.get("league_id") != expected_league_id:
            logging.error(
                f"[DELETE_TOKEN] League ID mismatch - Token: {payload.get('league_id')}, Expected: {expected_league_id}"
            )
        ...
        if payload.get("target_event_id") != expected_target_event_id:
            logging.error(
                f"[DELETE_TOKEN] Target event ID mismatch - Token: {payload.get('target_event_id')}, Expected: {expected_target_event_id}"
            )
        ...
        if mark_as_used:
            ...
            logging.info(
                f"[DELETE_TOKEN] Token marked as used (jti: {jti}) - Replay now blocked"
            )

        logging.info(
            f"[DELETE_TOKEN] Valid one-time token (jti: {jti}) for user {expected_user_id}, event {expected_target_event_id}"
        )
        ...
        logging.warning(
            f"[DELETE_TOKEN] Expired token for user {expected_user_id}, event {expected_target_event_id}"
        )
        raise
...
def cleanup_expired_tokens():
    ...
    now = datetime.utcnow()
    expired_jtis = [
        jti for jti, record in _token_usage_store.items() if record["expires_at"] < now
    ]
    ...
    if expired_jtis:
        logging.info(
            f"[DELETE_TOKEN] Cleaned up {len(expired_jtis)} expired tokens from usage store"
        )
