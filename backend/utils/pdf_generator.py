...
def generate_event_pdf(
    event_data: Dict[str, Any], stats: Dict[str, Any], players: List[Dict[str, Any]]
) -> BytesIO:
    """
    ...
    # Title Section
    elements.append(
        Paragraph(f"Combine Results: {event_data.get('name', 'Event')}", title_style)
    )

    date_str = event_data.get("date", "N/A")
    location_str = event_data.get("location", "N/A")
    elements.append(
        Paragraph(f"Date: {date_str} | Location: {location_str}", styles["Normal"])
    )
    ...
    elements.append(
        Paragraph(f"Total Participants: {stats['participant_count']}", styles["Normal"])
    )
    ...
    for i, drill in enumerate(sorted_drills):
        ...
        if data["count"] > 0:
            content = [
                ...
                Paragraph(
                    f"Best: {data['top_performers'][0]['value'] if data['top_performers'] else '-'}",
                    styles["Normal"],
                ),
    ...
    sorted_players = sorted(
        players, key=lambda p: (p.get("last_name", ""), p.get("first_name", ""))
    )
    ...
    elements.append(
        Paragraph(
            f"Generated by WooCombine on {datetime.now().strftime('%Y-%m-%d %H:%M')}",
            styles["Italic"],
        )
    )
